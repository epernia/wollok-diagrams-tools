...
| Tool | Function |
|---|---|
| [`wollokdd2drawio`](tools/wollokdd2drawio/README.md) | Takes a `.wlk` file and —if present— a `.wrepl` file (a custom file containing a program sequence one would run in the REPL) and executes them to generate a **Dynamic Diagram** in `.drawio` format. It can generate a single diagram showing a "snapshot of the entire environment" after full execution, or—by adding the `--genseq` argument—generate multiple pages showing the step-by-step execution of each line, grouping lines related to object creation on the same page. |
| [`wolloksd2drawio`](tools/wolloksd2drawio/README.md) | Generates a **Static Diagram** in `.drawio` format from a `.wlk` file. If the diagram is manually modified and the tool is run again to regenerate it, **manually adjusted positions are preserved**. |
| [`wolloksd2puml`](tools/wolloksd2puml/README.md) | Generates a **Static Diagram** in `.puml` format from a `.wlk` file. This diagram is text-based and handles Git diffs well. This format was used in some Wollok course notes. | 
